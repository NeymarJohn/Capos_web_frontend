...
const defaultFields = ['name', 'groupId', 'email', 'contact', 'physical_address_street', 'physical_address_city',
                     'physical_address_suburb', 'physical_address_postcode', 'physical_address_state', 'physical_address_country',
  ...
  import(file): void {
    if (file && file.name.split('.')[1] !== 'csv') {
      ...
      this.ngxCsvParser.parse(file, {header: this.header, delimiter: ','})
        ...
        this.utilService.post('customers/employee', data).subscribe(() => {
